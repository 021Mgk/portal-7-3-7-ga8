...
package mgkportlet.service.persistence.impl;
...
import mgkportlet.model.impl.PersonImpl;
import mgkportlet.model.impl.PersonModelImpl;

import mgkportlet.service.persistence.PersonPersistence;
import mgkportlet.service.persistence.impl.constants.mgkPersistenceConstants;
...
public class PersonPersistenceImpl
	...
	private FinderPath _finderPathWithPaginationFindByNationalCode;
	private FinderPath _finderPathWithoutPaginationFindByNationalCode;
	private FinderPath _finderPathCountByNationalCode;
	...
	public List<Person> findByNationalCode(String nationalCode) {
		return findByNationalCode(
			nationalCode, QueryUtil.ALL_POS, QueryUtil.ALL_POS, null);
	}
	 ...
	 */
	@Override
	public List<Person> findByNationalCode(
		String nationalCode, int start, int end) {

		return findByNationalCode(nationalCode, start, end, null);
	}
	...
	public List<Person> findByNationalCode(
		...
		return findByNationalCode(
			nationalCode, start, end, orderByComparator, true);
	}
	...
	public List<Person> findByNationalCode(
		...
		nationalCode = Objects.toString(nationalCode, "");
		...
		if ((start == QueryUtil.ALL_POS) && (end == QueryUtil.ALL_POS) &&
			...
			if (useFinderCache) {
				finderPath = _finderPathWithoutPaginationFindByNationalCode;
				finderArgs = new Object[] {nationalCode};
			}
		}
		else if (useFinderCache) {
			finderPath = _finderPathWithPaginationFindByNationalCode;
			finderArgs = new Object[] {
		...
		if (useFinderCache) {
			...
			if ((list != null) && !list.isEmpty()) {
				for (Person person : list) {
					if (!nationalCode.equals(person.getNationalCode())) {
						list = null;
		...
		if (list == null) {
			...
			boolean bindNationalCode = false;

			if (nationalCode.isEmpty()) {
				sb.append(_FINDER_COLUMN_NATIONALCODE_NATIONALCODE_3);
			}
			else {
				bindNationalCode = true;

				sb.append(_FINDER_COLUMN_NATIONALCODE_NATIONALCODE_2);
			}
	...
	public Person findByNationalCode_First(
		...
		Person person = fetchByNationalCode_First(
	...
	public Person fetchByNationalCode_First(
		...
		List<Person> list = findByNationalCode(
			nationalCode, 0, 1, orderByComparator);
	...
	public Person findByNationalCode_Last(
		...
		Person person = fetchByNationalCode_Last(
			nationalCode, orderByComparator);
	...
	public Person fetchByNationalCode_Last(
		...
		int count = countByNationalCode(nationalCode);
		...
		List<Person> list = findByNationalCode(
			nationalCode, count - 1, count, orderByComparator);
	...
	public Person[] findByNationalCode_PrevAndNext(
		...
		try {
			...
			array[0] = getByNationalCode_PrevAndNext(
				session, person, nationalCode, orderByComparator, true);

			array[1] = person;

			array[2] = getByNationalCode_PrevAndNext(
				session, person, nationalCode, orderByComparator, false);
	...
	protected Person getByNationalCode_PrevAndNext(
		Session session, Person person, String nationalCode,
		OrderByComparator<Person> orderByComparator, boolean previous) {
	...
	public int countByNationalCode(String nationalCode) {
		...
		FinderPath finderPath = _finderPathCountByNationalCode;

		Object[] finderArgs = new Object[] {nationalCode};
	...
	private static final String _FINDER_COLUMN_NATIONALCODE_NATIONALCODE_2 =
		"person.nationalCode = ?";

	private static final String _FINDER_COLUMN_NATIONALCODE_NATIONALCODE_3 =
		"(person.nationalCode IS NULL OR person.nationalCode = '')";
	 ...
	 */
	@Override
	public List<Person> findByGroupId(
	...
	public Person fetchByGroupId_Last(
		...
		List<Person> list = findByGroupId(
	...
	public Person[] findByGroupId_PrevAndNext(
		...
		try {
			...
			array[0] = getByGroupId_PrevAndNext(
				session, person, groupId, orderByComparator, true);

			array[1] = person;

			array[2] = getByGroupId_PrevAndNext(
				session, person, groupId, orderByComparator, false);
	...
	protected Person getByGroupId_PrevAndNext(
		Session session, Person person, long groupId,
		OrderByComparator<Person> orderByComparator, boolean previous) {
	 ...
	 */
	@Override
	public List<Person> filterFindByGroupId(
		long groupId, int start, int end,
		OrderByComparator<Person> orderByComparator) {
		...
		if (getDB().isSupportsInlineDistinct()) {
			sb.append(_FILTER_SQL_SELECT_PERSON_WHERE);
		}
		else {
			sb.append(_FILTER_SQL_SELECT_PERSON_NO_INLINE_DISTINCT_WHERE_1);
		}

		sb.append(_FINDER_COLUMN_GROUPID_GROUPID_2);

		if (!getDB().isSupportsInlineDistinct()) {
			sb.append(_FILTER_SQL_SELECT_PERSON_NO_INLINE_DISTINCT_WHERE_2);
		}
		...
		if (orderByComparator != null) {
			...
			if (getDB().isSupportsInlineDistinct()) {
				...
				sb.append(PersonModelImpl.ORDER_BY_SQL);
			}
		}

		String sql = InlineSQLHelperUtil.replacePermissionCheck(
			sb.toString(), Person.class.getName(),
		...
		try {
			...
			if (getDB().isSupportsInlineDistinct()) {
				sqlQuery.addEntity(_FILTER_ENTITY_ALIAS, PersonImpl.class);
			}
			else {
				sqlQuery.addEntity(_FILTER_ENTITY_TABLE, PersonImpl.class);
			}
			...
			return (List<Person>)QueryUtil.list(
	...
	protected Person filterGetByGroupId_PrevAndNext(
		Session session, Person person, long groupId,
		OrderByComparator<Person> orderByComparator, boolean previous) {
	...
	public int filterCountByGroupId(long groupId) {
		...
		sb.append(_FILTER_SQL_COUNT_PERSON_WHERE);
	...
	public void activate(BundleContext bundleContext) {
		...
		_finderPathWithPaginationFindByNationalCode = _createFinderPath(
			FINDER_CLASS_NAME_LIST_WITH_PAGINATION, "findByNationalCode",
			new String[] {
				String.class.getName(), Integer.class.getName(),
				Integer.class.getName(), OrderByComparator.class.getName()
			},
			new String[] {"nationalCode"}, true);

		_finderPathWithoutPaginationFindByNationalCode = _createFinderPath(
			FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION, "findByNationalCode",
			new String[] {String.class.getName()},
			new String[] {"nationalCode"}, true);

		_finderPathCountByNationalCode = _createFinderPath(
			FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION, "countByNationalCode",
			new String[] {String.class.getName()},
			new String[] {"nationalCode"}, false);
	...
	@Override
	@Reference(
		target = mgkPersistenceConstants.SERVICE_CONFIGURATION_FILTER,
		unbind = "-"
	)
	...
	@Override
	@Reference(
		target = mgkPersistenceConstants.ORIGIN_BUNDLE_SYMBOLIC_NAME_FILTER,
		unbind = "-"
	)
	...
	private static final String _FILTER_SQL_SELECT_PERSON_WHERE =
		"SELECT DISTINCT {person.*} FROM mgk_Person person WHERE ";

	private static final String
		_FILTER_SQL_SELECT_PERSON_NO_INLINE_DISTINCT_WHERE_1 =
			"SELECT {mgk_Person.*} FROM (SELECT DISTINCT person.personId FROM mgk_Person person WHERE ";

	private static final String
		_FILTER_SQL_SELECT_PERSON_NO_INLINE_DISTINCT_WHERE_2 =
			") TEMP_TABLE INNER JOIN mgk_Person ON TEMP_TABLE.personId = mgk_Person.personId";

	private static final String _FILTER_SQL_COUNT_PERSON_WHERE =
		"SELECT COUNT(DISTINCT person.personId) AS COUNT_VALUE FROM mgk_Person person WHERE ";

	private static final String _FILTER_ENTITY_ALIAS = "person";

	private static final String _FILTER_ENTITY_TABLE = "mgk_Person";

	private static final String _ORDER_BY_ENTITY_ALIAS = "person.";

	private static final String _ORDER_BY_ENTITY_TABLE = "mgk_Person.";
	...
	static {
		try {
			Class.forName(mgkPersistenceConstants.class.getName());
		}
